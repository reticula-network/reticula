...
consteval bool tuple_elems_eq_impl(std::index_sequence<I...>) {
  return (std::equality_comparable<std::tuple_element_t<I, T>> && ...);
}

template <class T>
concept tuple_elems_eq =
  is_tuple_like_v<T> &&
  tuple_elems_eq_impl<T>(std::make_index_sequence<std::tuple_size_v<T>>{});

// this is for gcc <= 14 issue: error: std::equality_comparable<T> returns true for
// pairs even if all parts are not comparable.
template <class T>
concept element_eq =
  (is_tuple_like_v<T> ? tuple_elems_eq<T>

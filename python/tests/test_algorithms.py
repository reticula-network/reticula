...
def test_temporal_clusters():
    ...
    net = ret.directed_temporal_network(edges)
    adj = ret.adjacency.simple()

    e = ret.directed_temporal_edge(1, 2, 1.5)
...
def test_cluster_functions():
    ...
    in_cluster = ret.in_cluster(net, 2, 1.5, adj)
    assert in_cluster.volume() == 1

    out_cluster = ret.out_cluster(net, 1, 1.5, adj)

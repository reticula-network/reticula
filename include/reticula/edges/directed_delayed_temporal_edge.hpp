...
namespace reticula {
/**
  A directed delayed edges (or event) indicate asymmertic temporal relation or
  orientation in the relationship between the two nodes. Unlike the
  instantaneous events (e.g. directed_temporal_etdge) the time of the start of
  the mutation is not equal to the time of the effect on the mutated vertices.

  You can also think of a directed delayed temporal edge as a "trip" of a bus
  between two consecutive stops in public transportation network or travel of a
  plane between two airports. In this case "the effect" is the arrival of
  passengers, the change is caused by departure of passengers from departure
  stop and the state of the destination stop is mutated by this effect.
*/
...
public:
  ...
  /**
    ...
    @param effect_time the time of the effect on the head vertex
   */
  directed_delayed_temporal_edge(
    VertexType tail, VertexType head, TimeType cause_time,
    TimeType effect_time);

  /**
    Create a directed delayed temporal edge from a static edge and times.
   ...
   */
  directed_delayed_temporal_edge(
    const StaticProjectionType& edge, TimeType cause_time,
    TimeType effect_time);
  ...
  auto operator<=>(const directed_delayed_temporal_edge& other) const = default;
...
private:
  ...
  friend struct edge_relabel<directed_delayed_temporal_edge>;
  friend struct std::hash<directed_delayed_temporal_edge>;
  friend struct hll::hash<directed_delayed_temporal_edge>;

  friend struct std::formatter<directed_delayed_temporal_edge, char>;
};
...
template <>
struct is_instantaneous<directed_delayed_temporal_edge> {
  static constexpr bool value = false;
};

template <>
struct is_undirected<directed_delayed_temporal_edge> {
  static constexpr bool value = false;
};

template <>
struct is_dyadic<directed_delayed_temporal_edge> {
  static constexpr bool value = true;
};

template <>
struct edge_relabel<directed_delayed_temporal_edge> {
  template <std::invocable<VertexType> MapF>
    requires std::convertible_to<
      std::invoke_result_t<MapF, VertexType>, VertexType>
  auto operator()(const directed_delayed_temporal_edge& e, MapF&& map_fun) const
    -> directed_delayed_temporal_edge {
    return directed_delayed_temporal_edge{
      map_fun(e.verts_[0]), map_fun(e.verts_[1]), e.cause_time_,
      e.effect_time_};
  }
};
} // namespace reticula
...
namespace std {
template <>
struct hash<reticula::directed_delayed_temporal_edge> {
  auto operator()(const reticula::directed_delayed_temporal_edge& e) const
    -> std::size_t {
...
template <>
struct formatter<reticula::directed_delayed_temporal_edge, char> {
  constexpr auto parse(format_parse_context& ctx) {
    auto it = ctx.begin();
    if (it != ctx.end() && *it == '}')
      return it;
    throw format_error(
      "Invalid format for reticula::directed_delayed_temporal_edge");
  }

  template <typename FormatContext>
  auto format(
    const reticula::directed_delayed_temporal_edge& e,
    FormatContext& ctx) const {
    return std::format_to(
      ctx.out(), "({} -> {} t_cause={} t_effect={})", e.verts_[0], e.verts_[1],
      e.cause_time_, e.effect_time_);
  }
};
} // namespace std
...
struct hash<reticula::directed_delayed_temporal_edge> {
  auto operator()(
    const reticula::directed_delayed_temporal_edge& e, std::uint64_t seed) const
    -> uint64_t {
    return hll::hash<std::size_t>{}(
      std::hash<reticula::directed_delayed_temporal_edge>{}(e), seed);
  }
};
} // namespace hll

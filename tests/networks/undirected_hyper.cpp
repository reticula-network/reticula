#include <ranges>
#include <vector>

#include <catch2/catch_test_macros.hpp>
#include <catch2/matchers/catch_matchers_contains.hpp>
#include <catch2/matchers/catch_matchers_range_equals.hpp>

using Catch::Matchers::Contains;
using Catch::Matchers::RangeEquals;
using Catch::Matchers::UnorderedRangeEquals;

#include "../stringify.hpp"

#include <reticula/concepts.hpp>
#include <reticula/networks.hpp>

TEST_CASE(
  "undirected hypernetwork complies with the concepts",
  "[reticula::network][reticula::undirected_hypernetwork]") {
  STATIC_REQUIRE(reticula::network_like<reticula::undirected_hypernetwork>);
  STATIC_REQUIRE(reticula::dense_id_network<reticula::undirected_hypernetwork>);
}

TEST_CASE(
  "constructing network from views",
  "[reticula::network][reticula::undirected_hypernetwork]") {
  std::uint64_t n = 5;
  auto verts = std::ranges::iota_view{0uz, n};
  auto edges = verts | std::views::transform([n](std::uint64_t v) {
                 return reticula::undirected_hyperedge({v, (v + 1) % n});
               });
  auto graph = reticula::undirected_hypernetwork(edges, verts);
  REQUIRE_THAT(
    graph.vertices(), UnorderedRangeEquals(std::vector<int>({0, 1, 2, 3, 4})));
  REQUIRE_THAT(
    graph.edges(), UnorderedRangeEquals(
                     std::vector<reticula::undirected_hyperedge>(
                       {{0, 1}, {1, 2}, {2, 3}, {3, 4}, {4, 0}})));
}

TEST_CASE(
  "undirected network edge list properties",
  "[reticula::network][reticula::undirected_hypernetwork]") {
  reticula::undirected_hypernetwork graph(
    {{1, 2},
     {1, 5},
     {5, 2},
     {4, 5},
     {3, 2},
     {4, 3},
     {4, 6},
     {1, 2},
     {2, 1},
     {1, 2},
     {5, 2}},
    {0});

  std::vector<reticula::undirected_hyperedge> edges(
    {{1, 2}, {1, 5}, {5, 2}, {4, 5}, {3, 2}, {4, 3}, {4, 6}});
  std::ranges::sort(edges);

  REQUIRE_THAT(graph.edges(), RangeEquals(edges));
  REQUIRE_THAT(graph.edges_cause(), RangeEquals(edges));
  REQUIRE_THAT(graph.edges_effect(), RangeEquals(edges));
}

TEST_CASE(
  "undirected network incidence list properties",
  "[reticula::network][reticula::undirected_hypernetwork]") {
  using vert_vec_t = std::vector<std::uint64_t>;

  reticula::undirected_hypernetwork graph(
    {{1, 2},
     {1, 5},
     {5, 2},
     {4, 5},
     {3, 2},
     {4, 3},
     {4, 6},
     {1, 2},
     {2, 1},
     {5, 2}},
    {0});

  REQUIRE_THAT(graph.successors(3), UnorderedRangeEquals(vert_vec_t({2, 4})));
  REQUIRE_THAT(graph.predecessors(3), UnorderedRangeEquals(vert_vec_t({2, 4})));
  REQUIRE_THAT(graph.neighbours(3), UnorderedRangeEquals(vert_vec_t({2, 4})));

  REQUIRE_THAT(
    graph.out_edges(3),
    UnorderedRangeEquals(
      std::vector<reticula::undirected_hyperedge>({{3, 4}, {3, 2}})));
  REQUIRE_THAT(
    graph.in_edges(3),
    UnorderedRangeEquals(
      std::vector<reticula::undirected_hyperedge>({{3, 4}, {3, 2}})));
  REQUIRE_THAT(
    graph.incident_edges(3),
    UnorderedRangeEquals(
      std::vector<reticula::undirected_hyperedge>({{3, 4}, {3, 2}})));

  REQUIRE(graph.out_degree(3) == 2);
  REQUIRE(graph.in_degree(3) == 2);
  REQUIRE(graph.degree(3) == 2);

  std::vector<reticula::undirected_hyperedge> edges(
    {{1, 2}, {1, 5}, {5, 2}, {4, 5}, {3, 2}, {4, 3}, {4, 6}});
  vert_vec_t verts({0, 1, 2, 3, 4, 5, 6});

  REQUIRE(graph == graph);
  REQUIRE(graph == reticula::undirected_hypernetwork(edges, verts));

  REQUIRE(
    graph != reticula::undirected_hypernetwork(
               std::vector<reticula::undirected_hyperedge>{{1, 2}}, verts));
  REQUIRE(graph != reticula::undirected_hypernetwork(edges, vert_vec_t{15}));

  REQUIRE_THAT(
    graph.vertices(), UnorderedRangeEquals(vert_vec_t({0, 1, 2, 3, 4, 5, 6})));
}

TEST_CASE(
  "undirected hypernetwork with true hyperedges",
  "[reticula::network][reticula::undirected_hypernetwork]") {
  reticula::undirected_hypernetwork graph(
    {{1, 2, 3}, {2, 3, 4, 5}, {4, 5, 6}, {1, 6, 7, 8, 9}});

  REQUIRE_THAT(
    graph.vertices(),
    UnorderedRangeEquals(std::vector<int>({1, 2, 3, 4, 5, 6, 7, 8, 9})));

  REQUIRE_THAT(
    graph.neighbours(2),
    UnorderedRangeEquals(std::vector<std::uint64_t>({1, 3, 4, 5})));
  REQUIRE_THAT(
    graph.neighbours(6),
    UnorderedRangeEquals(std::vector<std::uint64_t>({1, 4, 5, 7, 8, 9})));

  REQUIRE(graph.degree(2) == 2);
  REQUIRE(graph.degree(1) == 2);
  REQUIRE(graph.degree(5) == 2);

  REQUIRE_THAT(
     graph.incident_edges(2),
     UnorderedRangeEquals(
       std::vector<reticula::undirected_hyperedge>({{1, 2, 3}, {2, 3, 4, 5}})));
}

TEST_CASE(
  "undirected hypernetwork has_vertex and has_edge methods",
  "[reticula::network][reticula::undirected_hypernetwork]") {
  reticula::undirected_hypernetwork graph(
    {{1, 2}, {1, 5}, {5, 2}, {4, 5}, {3, 2}, {4, 3}, {4, 6}},
    {0});

  REQUIRE(graph.has_vertex(0));
  REQUIRE(graph.has_vertex(1));
  REQUIRE(graph.has_vertex(2));
  REQUIRE(graph.has_vertex(3));
  REQUIRE(graph.has_vertex(4));
  REQUIRE(graph.has_vertex(5));
  REQUIRE(graph.has_vertex(6));
  REQUIRE_FALSE(graph.has_vertex(7));
  REQUIRE_FALSE(graph.has_vertex(10));

  REQUIRE(graph.has_edge({1, 2}));
  REQUIRE(graph.has_edge({2, 1}));
  REQUIRE(graph.has_edge({1, 5}));
  REQUIRE(graph.has_edge({5, 1}));
  REQUIRE(graph.has_edge({5, 2}));
  REQUIRE(graph.has_edge({2, 5}));
  REQUIRE(graph.has_edge({4, 5}));
  REQUIRE(graph.has_edge({5, 4}));
  REQUIRE(graph.has_edge({3, 2}));
  REQUIRE(graph.has_edge({2, 3}));
  REQUIRE(graph.has_edge({4, 3}));
  REQUIRE(graph.has_edge({3, 4}));
  REQUIRE(graph.has_edge({4, 6}));
  REQUIRE(graph.has_edge({6, 4}));
  REQUIRE_FALSE(graph.has_edge({1, 3}));
  REQUIRE_FALSE(graph.has_edge({1, 4}));
  REQUIRE_FALSE(graph.has_edge({7, 8}));

  reticula::undirected_hypernetwork hyper_graph(
    {{1, 2, 3}, {2, 3, 4, 5}, {4, 5, 6}, {1, 6, 7, 8, 9}});

  REQUIRE(hyper_graph.has_edge({1, 2, 3}));
  REQUIRE(hyper_graph.has_edge({2, 3, 4, 5}));
  REQUIRE(hyper_graph.has_edge({4, 5, 6}));
  REQUIRE(hyper_graph.has_edge({1, 6, 7, 8, 9}));
  REQUIRE_FALSE(hyper_graph.has_edge({1, 2}));
  REQUIRE_FALSE(hyper_graph.has_edge({1, 2, 3, 4}));
}
...
TEST_CASE(
  "directed temporal network complies with the concepts",
  "[reticula::network][reticula::directed_temporal_network]") {
  STATIC_REQUIRE(reticula::network_like<reticula::directed_temporal_network>);
  STATIC_REQUIRE(reticula::dense_id_network<reticula::directed_temporal_network>);
}
...
TEST_CASE(
  ...
  auto edges = verts | std::views::transform([n](std::uint64_t v) {
                 return reticula::directed_temporal_edge(v, (v + 1) % n, static_cast<double>(v));
               });
  auto graph = reticula::directed_temporal_network(edges, verts);
  REQUIRE_THAT(
    graph.vertices(), UnorderedRangeEquals(std::vector<int>({0, 1, 2, 3, 4})));
  REQUIRE_THAT(
    graph.edges(), UnorderedRangeEquals(
                     std::vector<reticula::directed_temporal_edge>(
                       {{0, 1, 0}, {1, 2, 1}, {2, 3, 2}, {3, 4, 3}, {4, 0, 4}})));
}

TEST_CASE(
  "directed temporal network edge list properties",
  "[reticula::network][reticula::directed_temporal_network]") {
  reticula::directed_temporal_network graph(
    {{1, 2, 10}, {2, 3, 20}, {3, 5, 30}, {5, 6, 40}, {5, 4, 35}, {4, 2, 25}, 
     {1, 2, 10}, {2, 3, 20}, {3, 5, 30}},
    {0});
...
TEST_CASE(
  "directed temporal network incidence list properties",
  "[reticula::network][reticula::directed_temporal_network]") {
  ...
  REQUIRE_THAT(
    graph.out_edges(2),
    UnorderedRangeEquals(std::vector<reticula::directed_temporal_edge>({{2, 3, 20}})));
  REQUIRE_THAT(
    graph.in_edges(2),
    UnorderedRangeEquals(
      std::vector<reticula::directed_temporal_edge>({{4, 2, 25}, {1, 2, 10}})));
  REQUIRE_THAT(
    graph.incident_edges(2),
    UnorderedRangeEquals(
      std::vector<reticula::directed_temporal_edge>({{4, 2, 25}, {1, 2, 10}, {2, 3, 20}})));
  ...
  REQUIRE(graph == graph);
  REQUIRE(graph == reticula::directed_temporal_network(edges, verts));

  REQUIRE(
    graph != reticula::directed_temporal_network(
               std::vector<reticula::directed_temporal_edge>{{1, 2, 10}, {2, 3, 20}}, verts));
  REQUIRE(graph != reticula::directed_temporal_network(edges, vert_vec_t{15}));
...
TEST_CASE(
  "directed temporal network temporal properties",
  "[reticula::network][reticula::directed_temporal_network]") {
  reticula::directed_temporal_network graph(
...
TEST_CASE(
  "directed temporal network has_vertex and has_edge methods",
  "[reticula::network][reticula::directed_temporal_network]") {
  reticula::directed_temporal_network graph(
    {{1, 2, 10}, {2, 3, 20}, {3, 5, 30}, {5, 6, 40}, {5, 4, 35}, {4, 2, 25}},
    {0});

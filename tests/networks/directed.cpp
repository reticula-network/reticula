...
TEST_CASE(
  "directed network complies with the concepts",
  "[reticula::network][reticula::directed_network]") {
  STATIC_REQUIRE(reticula::network_like<reticula::directed_network>);
  STATIC_REQUIRE(reticula::dense_id_network<reticula::directed_network>);
}
...
TEST_CASE(
  ...
  auto graph = reticula::directed_network(edges, verts);
  REQUIRE_THAT(
...
TEST_CASE(
  ...
  reticula::directed_network graph(
    {{1, 2}, {2, 3}, {3, 5}, {5, 6}, {5, 4}, {4, 2}, {1, 2}, {2, 3}, {3, 5}},
    {0});
...
TEST_CASE(
  ...
  REQUIRE_THAT(
    graph.out_edges(2),
    UnorderedRangeEquals(std::vector<reticula::directed_edge>({{2, 3}})));
  REQUIRE_THAT(
    graph.in_edges(2),
    UnorderedRangeEquals(
      std::vector<reticula::directed_edge>({{4, 2}, {1, 2}})));
  REQUIRE_THAT(
    graph.incident_edges(2),
    UnorderedRangeEquals(
      std::vector<reticula::directed_edge>({{4, 2}, {1, 2}, {2, 3}})));
  ...
  REQUIRE(graph == graph);
  REQUIRE(graph == reticula::directed_network(edges, verts));

  REQUIRE(
    graph != reticula::directed_network(
               std::vector<reticula::directed_edge>{{1, 2}, {2, 3}}, verts));
  REQUIRE(graph != reticula::directed_network(edges, vert_vec_t{15}));
...
TEST_CASE(
  "directed network has_vertex and has_edge methods",
  "[reticula::network][reticula::directed_network]") {
  reticula::directed_network graph(
    {{1, 2}, {2, 3}, {3, 5}, {5, 6}, {5, 4}, {4, 2}},
    {0});

...
TEST_CASE(
  "undirected network complies with the concepts",
  "[reticula::network][reticula::directed_network]") {
  STATIC_REQUIRE(reticula::network_like<reticula::undirected_network>);
  STATIC_REQUIRE(reticula::dense_id_network<reticula::undirected_network>);
}

TEST_CASE(
  "constructing network from views",
  "[reticula::network][reticula::undirected_network]") {
  std::uint64_t n = 5;
  auto verts = std::ranges::iota_view{0uz, n};
  auto edges = verts | std::views::transform([n](std::uint64_t v) {
                 return reticula::undirected_edge(v, (v + 1) % n);
               });
  auto graph = reticula::undirected_network(edges, verts);
  REQUIRE_THAT(
...
TEST_CASE(
  ...
  REQUIRE_THAT(
    ...
    UnorderedRangeEquals(
      std::vector<reticula::undirected_edge>({{3, 4}, {3, 2}})));
  REQUIRE_THAT(
    graph.in_edges(3),
    UnorderedRangeEquals(
      std::vector<reticula::undirected_edge>({{3, 4}, {3, 2}})));
  ...
  REQUIRE(graph == graph);
  REQUIRE(graph == reticula::undirected_network(edges, verts));

  REQUIRE(
    graph != reticula::undirected_network(
               std::vector<reticula::undirected_edge>{{1, 2}}, verts));
  REQUIRE(graph != reticula::undirected_network(edges, vert_vec_t{15}));
...
TEST_CASE(
  "undirected network has_vertex and has_edge methods",
  "[reticula::network][reticula::undirected_network]") {
  reticula::undirected_network graph(
    {{1, 2}, {1, 5}, {5, 2}, {4, 5}, {3, 2}, {4, 3}, {4, 6}},
    {0});
